...
class CoNLLeval:
    """Codes borrowed from: https://github.com/AdolfVonKleist/rnn-slu/blob/master/rnnslu/CoNLLeval.py
    ...
    # options:   l: generate LaTeX output for tables like in http://cnts.uia.ac.be/conll2003/ner/example.tex
    #            r: accept raw result tags (without B- and I- prefix; assumes one word per chunk)
    #            d: alternative delimiter tag (default is single space)
    #            o: alternative outside tag (default is O)
    # note:      the file should contain lines with items separated by $delimiter characters (default space). The final
    #            two items should contain the correct tag and the guessed tag in that order. Sentences should be
    #            separated from each other by empty lines or lines with $boundary fields (default -X-).
    ...
    @staticmethod
    def end_of_chunk(prev_tag, tag, prev_type, tag_type, chunk_end=0):
    ...
    @staticmethod
    def start_of_chunk(prev_tag, tag, prev_type, tag_type, chunk_start=0):
        ...
        if prev_tag == "O" and tag == "I":
            chunk_start = True
        if tag != "O" and tag != "." and prev_type != tag_type:
            chunk_start = True
        # corrected 1998-12-22: these chunks are assumed to have length 1
        if tag == "[":
            chunk_start = True
        ...
        with open(infile, "r") as ifp:
            for line in ifp:
                ...
                if self.in_correct is True:
                    if self.end_of_chunk(self.last_correct, self.correct, self.last_correct_type, self.correct_type) \
                            is True and self.end_of_chunk(self.last_guessed, self.guessed, self.last_guessed_type,
                                                          self.guessed_type) is True and \
                            self.last_guessed_type == self.last_correct_type:
                        self.in_correct = False
                        self.correct_chunk[self.last_correct_type] += 1
                    elif self.end_of_chunk(self.last_correct, self.correct, self.last_correct_type,
                                           self.correct_type) != self.end_of_chunk(self.last_guessed, self.guessed,
                                                                                   self.last_guessed_type,
                                                                                   self.guessed_type) \
                            or self.guessed_type != self.correct_type:
                        self.in_correct = False

                if self.start_of_chunk(self.last_correct, self.correct, self.last_correct_type, self.correct_type) is \
                        True and self.start_of_chunk(self.last_guessed, self.guessed, self.last_guessed_type,
                                                     self.guessed_type) is True and self.guessed_type == \
                        self.correct_type:
                    self.in_correct = True

                if self.start_of_chunk(self.last_correct, self.correct, self.last_correct_type,
                                       self.correct_type) is True:
                    self.found_correct[self.correct_type] += 1

                if self.start_of_chunk(self.last_guessed, self.guessed, self.last_guessed_type,
